...
document.addEventListener('click', (event) => {
    const isNGOElement = event.target.closest('.ngo-card') || event.target.closest('.main-nav .search-bar input');
    if (!isNGOElement) {
        resetNGOs();
    }
});
...
function searchNGOs() {
    ...
    const searchValue = document.querySelector('.main-nav .search-bar input').value.toLowerCase().trim();
